...
export function openContextMenu(event) {
  ...
  let open = target.id === 'page-list' || target.id === 'floor-list' || target.classList.contains('background-image') || target.dataset.id || target.parentElement.dataset.page || target.parentElement.dataset.floor || target.dataset.jump;
