...
export function onCreateJump() {
  let jump = new Jump(
    ...
    global.state.createJumpIcon.id,
    global.state.selectedFloor,
    global.state.createJumpFloor
  )
...
function populateFloorList() {
  ...
  let floors = global.map.floors
    .filter(f => f.page === global.state.selectedPage)
    .filter(f => f.id !== global.state.selectedFloor)
...
function onInputChange() {
  let button = document.getElementById('create-jump') as HTMLButtonElement
